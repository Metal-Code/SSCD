    ...
    public void run() {
        for (List<String> currentLine : parsedCode) {
            ...
            if (Arrays.stream(OpCode.values()).anyMatch
                    (c -> c.name().equalsIgnoreCase(currentLine.get(0)))) {
                OpCode currentOpCode = getOpCode(currentLine.get(0));
                ...
                if (!symbolTable.containsKey(currentLine.get(0))) {
                    currentSymbol = new Symbol(currentLine.get(0), locationCounter, 1, symbolTableIndex++);
                    symbolTable.put(currentLine.get(0), currentSymbol);
                } else {
                    currentSymbol = symbolTable.get(currentLine.get(0));
...
//        assignment12.readInputFile("Assignment_1_2/input.asc");
        assignment12.readInputFile("input.asc");
        assignment12.run();
    }
}